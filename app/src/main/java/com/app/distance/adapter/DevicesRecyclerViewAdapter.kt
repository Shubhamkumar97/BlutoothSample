...
import com.app.distance.data.model.DeviceData
...
class DevicesRecyclerViewAdapter(val mDeviceList: List<DeviceData>, val context: Context) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val view =
            LayoutInflater.from(context).inflate(R.layout.recyclerview_single_item, parent, false)
        return VH(view)
    }
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        holder.label?.text =
            mDeviceList[position].deviceName
//                ?: mDeviceList[position].deviceHardwareAddress
    ...
    inner class VH(itemView: View?) : RecyclerView.ViewHolder(itemView!!) {
        ...
        init {
            itemView?.setOnClickListener {
                listener?.itemClicked(mDeviceList[adapterPosition],context)
            }
        }
    }
    ...
    interface ItemClickListener {
        fun itemClicked(deviceData: DeviceData,context: Context)
    }
